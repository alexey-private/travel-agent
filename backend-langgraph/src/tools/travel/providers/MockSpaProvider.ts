...
import { SpaProvider, SpaSearchParams, SpaOffer } from './SpaProvider';
...
const TREATMENT_MENUS: Record<string, { signature: string; treatments: string[] }> = {
  massage: { signature: 'Hot Stone Full Body Massage', treatments: ['Swedish Massage', 'Deep Tissue Massage', 'Hot Stone Massage', 'Thai Massage', 'Aromatherapy Massage', 'Sports Massage', 'Couples Massage'] },
  facial: { signature: 'Anti-Aging Collagen Facial', treatments: ['Hydrating Facial', 'Anti-Aging Facial', 'Deep Cleansing Facial', 'Microdermabrasion', 'LED Light Therapy', 'Chemical Peel', 'Gold Facial'] },
  body: { signature: 'Mediterranean Salt Scrub & Wrap', treatments: ['Salt Scrub', 'Body Wrap', 'Mud Treatment', 'Exfoliation', 'Body Polishing', 'Slimming Wrap', 'Detox Body Treatment'] },
  wellness: { signature: 'Full Day Wellness Journey', treatments: ['Yoga Session', 'Meditation', 'Sound Bath', 'Nutrition Consultation', 'Breathwork', 'Reiki', 'Crystal Healing', 'Infrared Sauna'] },
  thermal: { signature: 'Thermal Circuit Experience', treatments: ['Steam Room', 'Finnish Sauna', 'Ice Pool', 'Tepidarium', 'Hammam', 'Roman Bath', 'Flotation Tank', 'Hydrotherapy'] },
  ayurveda: { signature: 'Abhyanga Ayurvedic Oil Massage', treatments: ['Shirodhara', 'Abhyanga', 'Panchakarma', 'Marma Therapy', 'Herbal Steam', 'Nasya', 'Ayurvedic Consultation'] },
};
...
export class MockSpaProvider implements SpaProvider {
  async search(params: SpaSearchParams): Promise<ToolResult> {
    const { city, treatmentType, maxPrice, maxResults } = params;
    ...
    for (let i = 0; i < SPA_NAMES.length && spas.length < count * 2; i++) {
      ...
      const spaName = SPA_NAMES[nameIdx] + (spas.some(s => s.name === SPA_NAMES[nameIdx]) ? ` ${city}` : '');

      spas.push({
        name: spaName,
        city,
        type: type.charAt(0).toUpperCase() + type.slice(1),
        rating: Math.round((4.0 + seededRand(seed, i * 17 + 3) * 1.0) * 10) / 10,
        reviewCount,
        priceRange: priceRange.label,
        signature: menu.signature,
        treatments: menu.treatments.slice(0, treatmentCount),
        amenities,
        durationOptions: DURATION_OPTIONS.slice(0, durationCount),
        address: `${streetNum} Wellness District, ${city}`,
        bookingRequired: seededRand(seed, i * 43 + 9) > 0.3,
        highlights: [
    ...
    if (spas.length === 0) {
      return {
        success: true,
        data: { spas: [], message: `No spas found in ${city} matching your criteria. Try adjusting filters.` },
      };
    }

    return {
      success: true,
      data: { spas, city, source: 'mock', note: 'Sample data for demonstration purposes. Prices and availability are illustrative.' },
    };
  }
}
