...
export interface MemoryPanelHandle {
  reload: () => void;
}
...
const MemoryPanel = forwardRef<MemoryPanelHandle, MemoryPanelProps>(
  function MemoryPanel({ userId, agentType }, ref) {
    const t = useT();
    const {
      data: memories,
      loading,
      ...
      reload: loadMemories,
      setData: setMemories,
    } = useAsync(() => fetchMemories(userId, agentType), [userId, agentType]);
    ...
    useImperativeHandle(ref, () => ({ reload: loadMemories }), [loadMemories]);
    ...
    return (
      <aside className="w-64 h-full border-s border-gray-200 flex flex-col bg-gray-50 shrink-0">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin p-3 space-y-2">
          {error && (
            <p className="text-xs text-red-500 text-center">{t(error as TKey)}</p>
          )}
          ...
          {memoryList.map((mem) => (
            ...
            >
              ...
              <button
                ...
                title={t("memory.deleteEntry", { key: mem.key })}
              >
                <Trash2 size={13} />
              </button>
            </div>
        ...
        <div className="border-t border-gray-200 p-3 bg-white">
          {providerSettings === null ? (
            <p className="text-xs text-gray-400">{t("memory.checking")}</p>
          ) : providerSettings.calendarProvider === "apple" ? (
            <>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <Calendar size={13} className="text-gray-500" />
                  <span className="text-xs font-semibold text-gray-600">{t("memory.appleICloud")}</span>
                </div>
              ...
              {providerSettings.appleConnected ? (
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-xs text-green-600">
                    <CheckCircle2 size={12} /> {t("memory.connected")}
                  </div>
                  <button
            ...
            <>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <Calendar size={13} className="text-gray-500" />
                  <span className="text-xs font-semibold text-gray-600">{t("memory.googleCalendar")}</span>
                </div>
                <Link href="/settings" className="text-gray-400 hover:text-gray-600">
                  <Settings size={12} />
                </Link>
