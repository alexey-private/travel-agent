...
Уточнение по API (исправлено при реализации): `bidi-js@1.0.3` **отдаёт** готовую
строку — `getReorderedString(text, levels)` разворачивает сегменты и подставляет
зеркальные символы. `toVisual` вызывает её; собирать строку из `getReorderSegments`
и `getMirroredCharactersMap` вручную, как предполагалось здесь раньше, не нужно.
...
**Как это проверять.** `pdftotext` применяет UBA к тексту, который уже переставлен,
и показывает `ב-793` там, где на странице стоит `397`, — его координатам верить
можно, содержимому нет. Порядок символов проверяется в точке записи: строка,
уходящая в `doc.text`, прогоняется через `toVisual` обратно и должна совпасть с
исходником. Отсутствие регрессии для LTR проверяется растеризацией: английский и
русский документы, отрисованные до и после изменений, обязаны совпадать
попиксельно.
